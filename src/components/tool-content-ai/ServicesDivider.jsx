                    ...
                    {services.map((service) => (
                        <div key={service.id} className="flex flex-col items-center text-center group">
                            <div className="relative mb-6">
                                {/* Ellipse Container */}
                                <div className={`relative w-[88%] mx-auto h-36 md:w-52 md:h-40 lg:w-80 lg:h-52 flex items-center justify-center transform ${service.ellipseRotation} ${service.hoverRotation} transition-transform duration-500`}>
                                    {/* Ellipse Border */}
                                    <div className="absolute inset-0 rounded-[30px] border-2 border-white transition-colors duration-300"></div>
                                    {/* Content Inside Ellipse */}
                                    <div className="px-4 py-8 text-white">
                                         <p className="text-[15px] lg:text-[16px] whitespace-pre-line mb-2 font-bold">
                                            {service.title}
                                        </p>
                                            <p className="text-[13px] lg:text-[16px] whitespace-pre-line">
                                            {service.description}
                                        </p>
                                    </div>
                                </div>
